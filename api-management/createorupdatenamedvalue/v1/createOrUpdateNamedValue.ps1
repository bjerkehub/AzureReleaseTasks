...
Trace-VstsEnteringInvocation $MyInvocation
    ...
    if($existingNamedValue){
        ...
        if($IsSecret){
            New-AzApiManagementNamedValue -NamedValueId $Id -Name $Name -Value $Value -Tag $tagArray -Secret
        }else{
            New-AzApiManagementNamedValue -NamedValueId $Id -Name $Name -Value $Value -Tag $tagArray
        }
        ...
        if($IsSecret){
            Set-AzApiManagementNamedValue -NamedValueId $Id -Name $Name -Value $Value -Tag $tagArray -Secret
        }else{
            Set-AzApiManagementNamedValue -NamedValueId $Id -Name $Name -Value $Value -Tag $tagArray
        }
